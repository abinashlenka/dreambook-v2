...
function userValidator(user) {
  console.log("Validating user:", user); // Debug log
...
async function updateUserById(id, newDetails, profileImage = null) {
  ...
  if (updates.password) {
    ...
    console.log(`Password updated in Firebase for UID: ${user.firebaseUid}`);
  }

  if (profileImage) {
    updates.profileImage = profileImage.path;
  }
...
async function updateAuthor(id, newDetails) {
  ...
  userValidator(author);

  let updates = { ...newDetails };

  if (updates.password) {
    await admin.auth().updateUser(author.firebaseUid, {
      password: updates.password,
    });
    console.log(`Password updated in Firebase for UID: ${author.firebaseUid}`);
  }
...
async function updateEmployee(id, newDetails) {
  ...
  userValidator(employee);

  let updates = { ...newDetails };

  if (updates.password) {
    await admin.auth().updateUser(employee.firebaseUid, {
      password: updates.password,
    });
  }
...
async function updateAdmin(id, newDetails) {
  const adminUser = await User.findById(id);
  userValidator(adminUser);
