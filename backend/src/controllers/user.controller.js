...
const updateEmployee = catchAsync(async (req, res) => {
  const updatedEmployee = await userService.updateEmployee(req.params.userId, req.body);
  res.status(200).json({ status: true, message: 'Employee details are updated', data: updatedEmployee });
});

const updateAdmin = catchAsync(async (req, res) => {
  const updatedAdmin = await userService.updateAdmin(req.params.userId, req.body);
  console.log("Updated Admin:", updatedAdmin); // Debug log
  res.status(200).json({ status: true, message: 'Admin details are updated', data: updatedAdmin });
});
